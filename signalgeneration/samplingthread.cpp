...
void samplingthread::sample(double elapsed)
{
    ...
    case 4:
        ...
        votages = RAMP(elapsed,1.0,0.10,500/1000.0,4*(10.0/1.0),0.0);
        waveformfunc(elapsed);
...
long double samplingthread::samplingthread::SINEWAVE(double timeStamp, double frequency, bool continuous, double amplitude, int cycle, double Bias)
{
    ...
    if(frequency > 0.0)
    {
        if((continuous) || !(cycle*period < timeStamp))
        {
            ...
            DATA = Bias + (amplitude * sin(x/period*2*M_PI));
            return DATA;
